...
class TransformerEncoder(Seq2SeqEncoder):
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        Without position embeddings, the transformer computes a function that is
        invariant with respect to the input sequence. Position embeddings introduce
        a dependence on the order in which words are introduced. LSTMs, on the other
        hand, are intrinsically dependents on the word order, since they read the
        sentence from one direction to the other.
        '''
...
class TransformerDecoder(Seq2SeqDecoder):
    ...
    def forward(self, tgt_inputs, encoder_out=None, incremental_state=None, features_only=False):
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            The self_attn_mask ensures that the model can not attend to subsequent
            target word tokens. Such that it only depends on the encoder output
            and previous target words. In the encoder, we do not apply such a
            mask since we want words to attend to the whole sentence. In incremental
            decoding, we do not need a mask as the decoder is only fed the previous
            output words, and thus has no access to subsequent words.
            '''
        ...
        if not features_only:
            ...
            What is the dimensionality of forward_state after this line?
            What would the output represent if features_only=True?

            This linear projection is needed to convert the decoder output to a
            distribution over the target vocabulary in order to output word tokens.
            The dimensionality of is thus [batch_size, tgt_time_steps, tgt_vocab_size].
            If features_only=True, the output would simply the output from the decoder,
            i.e. a mini-batch of contextualized word embeddings.
