...
class LSTMEncoder(Seq2SeqEncoder):
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        At time step t, the final hidden states form a representation, and the
        final cell states control what the hidden states should remember or forget.
        If self.bidirectional is set to True, each input is processed in two directions
        (left-to-right and right-to-left), and the two directions of the final states
        are concatenated to form a single representation of the input sequence for
        each time step.
        '''
        if self.bidirectional:
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, tgt_input, encoder_out, src_mask):
        ...
        The attention context vector is calculated by applying the softmax function on
        the attention scores and using the resulting weights to average over the source
        hidden states. We need a mask to pad shorter sentences with -inf so that their
        weights are mapped to 0 after applying the softmax function.
        '''
    ...
    def score(self, tgt_input, encoder_out):
        ...
        Attention scores are calculated according to the general scoring function from
        Luong et al (2015). The matrix multiplication computes the dot products between
        the target input and the linearly projected outputs of the encoder in batches, i.e.
        [batch, 1, tgt_dims] * [batch, tgt_dims, src_length] ---> [batch, 1, src_length]}
        '''
...
class LSTMDecoder(Seq2SeqDecoder):
    ...
    def __init__(self,
        ...
        if self.use_lexical_model:
            ...
            # TODO: --------------------------------------------------------------------- CUT
            self.skip_projection = nn.Linear(embed_dim, embed_dim, bias = False)
            self.second_final_projection = nn.Linear(embed_dim, len(dictionary))
            # TODO: --------------------------------------------------------------------- /CUT
    ...
    def forward(self, tgt_inputs, encoder_out, incremental_state=None):
        ...
        If we do not use incremental decoding, cached_state is set to None and
        the decoder states are initialized as zero (otherwise it is initialized
        as the previous cached state). Similar to Luong et al (2015), input feed
        is the output from previous time step after global attention, which
        informs the model about past alignment decisions.
        '''
        ...
        for j in range(tgt_time_steps):
            ...
            target state as one of its inputs? What is the purpose of the dropout layer?
             ...
             Attention is integrated into the decoder by projecting the "previous"
             hidden state concatenated with the context vector. The attention function
             takes "previous" decoder hidden state ("previous" refers to the last layer
             of stacked LSTMs, not the previous time step) as input in order to compare
             it to the encoder output layer. The dropout layer prevents overfitting
             by randomly setting units to zero. 
            '''
            if self.attention is None:
                ...
                if self.use_lexical_model:
                    ...
                    # TODO: --------------------------------------------------------------------- CUT
                    """
                    In the article from Nguyen and Chang (2018), they use slightly different variables.
                    They correspond to our variables as:

                    f_s: source_words
                    a_t(s): attention
                    f_t^l: weighted_avg
                    h_t^l: context_vector.

                    The correspondence is exact when considering only the "final" version of our variables.
                    """
                    source_words = src_embeddings.transpose(0,1)         #[batch_size, src_time_steps, embed_dim]
                    attention = step_attn_weights.unsqueeze(1)           #[batch_size, 1, src_time_steps]
                    weighted_avg = torch.bmm(attention, source_words)    #[batch_size, 1, embed_dim]
                    weighted_avg = weighted_avg.squeeze(1)               #[batch_size, embed_dim]
                    weighted_avg = torch.tanh(weighted_avg)                  #[batch_size, embed_dim]
                    context_vector = self.skip_projection(weighted_avg)  #[batch_size, embed_dim]
                    context_vector = torch.tanh(context_vector)              #[batch_size, embed_dim]
                    context_vector = context_vector + weighted_avg       #[batch_size, embed_dim]
        ...
        if self.use_lexical_model:
            ...
            #Convert list to tensor (looks more complicated than it is)
            lexical_output = torch.cat(lexical_contexts, dim=0).view(tgt_time_steps, batch_size, self.embed_dim)

            #Transpose back to because we want batch_size as first dimension (see above)
            lexical_output = lexical_output.transpose(0, 1)

            #Project. Nguyen and Chang (2018) call the corresponding weights and biases W^l and b^l
            lexical_output = self.second_final_projection(lexical_output)

            #Sum outputs of projections together
